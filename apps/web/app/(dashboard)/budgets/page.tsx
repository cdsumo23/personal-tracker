...
function BudgetCard({
  ...
  return (
    <Card className="hover:border-slate-700/60 transition-all duration-300 relative group">
      <div className="flex items-start justify-between mb-4">
        <div>
          <h3 className="font-bold text-slate-100 text-sm">{budget.name}</h3>
        ...
        <div className="flex items-center gap-2">
          ...
          <div className="relative">
            <button
              ...
              className="p-1.5 rounded-lg text-slate-500 hover:text-slate-300 hover:bg-slate-800/60 opacity-0 group-hover:opacity-100 transition-all"
            >
              <MoreVertical className="w-4 h-4" />
            </button>
            {showMenu && (
              <div className="absolute right-0 top-8 z-10 bg-slate-900 border border-slate-800 rounded-xl shadow-2xl overflow-hidden py-1 w-32">
                <button onClick={() => { setShowMenu(false); onEdit(budget); }} className="w-full flex items-center gap-2.5 px-3 py-2 text-xs text-slate-300 hover:bg-slate-800">
        ...
        <>
          <div className="flex justify-between items-end mb-2">
            <div>
              ...
              <p className="text-lg font-extrabold text-slate-100">{format(usage.spent)}</p>
                    ...
                    <div className="flex justify-between text-[10px] text-slate-500 mb-1">
                      ...
                      <span className="flex items-center gap-1">
                        ...
                        {cat.currency && cat.currency !== 'USD' && (
                          <span className="bg-slate-700/60 text-slate-400 rounded px-1 py-0.5 text-[9px] font-bold">{cat.currency}</span>
                        )}
                      </span>
                    </div>
    ...
    <div className="space-y-6">
      ...
      {budgets.filter((b) => b.isActive).length > 0 && (
        <Card className="flex items-center gap-4 py-4">
          ...
          <div>
            <p className="text-xs font-bold text-slate-400 uppercase tracking-wider">Total Budgeted This Period</p>
            <p className="text-xl font-extrabold text-slate-100">{format(totalAllocated)}</p>
          </div>
        </Card>
      )}
        ...
        <form onSubmit={handleSubmit(onSubmit)} className="space-y-5">
          ...
          <div>
            <div className="flex items-center justify-between mb-3">
              <label className="text-xs font-bold text-slate-400 uppercase tracking-wider">Category Allocations</label>
            ...
            <div className="space-y-3 max-h-60 overflow-y-auto pr-1">
              {fields.map((field, index) => (
                <div key={field.id} className="space-y-2 p-3 bg-slate-800/30 border border-slate-700/40 rounded-xl">
                  {/* Category select — full width */}
                  <select
                    {...register(`categories.${index}.categoryId`)}
                    className="w-full bg-slate-800/60 border border-slate-700/50 rounded-xl px-3 py-2 text-sm text-slate-200 focus:outline-none focus:ring-1 focus:ring-primary-500"
                  >
                  ...
                  <div className="flex items-center gap-2">
                    ...
                    <select
                      ...
                      className="w-20 bg-slate-800/60 border border-slate-700/50 rounded-xl px-2 py-2 text-sm text-slate-200 focus:outline-none focus:ring-1 focus:ring-primary-500"
                    >
                      {CURRENCIES.map((cur) => (

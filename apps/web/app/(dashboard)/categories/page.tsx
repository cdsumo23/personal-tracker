    ...
    <div className="space-y-6 pb-20">
      ...
      {isLoading ? (
        <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-4">
          {[1, 2, 3, 4, 5, 6].map((n) => (
            <Card key={n} className="h-24 animate-pulse bg-slate-800/40 border border-slate-700" />
          ))}
        </div>
        ...
        <Card className="flex flex-col items-center justify-center p-12 text-center border-slate-800 bg-slate-900/40">
          ...
          <h3 className="text-lg font-semibold text-slate-200">No categories found</h3>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-4">
          {filteredCategories.map((cat) => {
            ...
            return (
              <Card
                key={cat.id}
                className="flex items-center justify-between p-4 bg-slate-900/60 border-slate-800/80 hover:border-slate-700/60 backdrop-blur-md transition-all group"
              >
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    ...
                    <div className="flex items-center gap-1.5 mt-0.5">
                      {isSystem ? (
                        <Badge variant="default" className="text-[9px] px-1 py-0 bg-slate-800 text-slate-400 border border-slate-750">
                          System
                        </Badge>
                      ) : (
        ...
        >
          <form onSubmit={handleSubmit(onSubmit)} className="space-y-4 pt-2">
            ...
            <div className="space-y-2">
              <label className="block text-sm font-medium text-slate-300">Theme Color</label>
              <div className="flex flex-wrap gap-2 p-3 bg-slate-950/40 border border-slate-800 rounded-xl">
            ...
            {/* ICON PICKER */}
            <div className="space-y-2">
              <label className="block text-sm font-medium text-slate-300">Category Icon</label>
              <div className="grid grid-cols-6 sm:grid-cols-8 gap-2 p-3 bg-slate-950/40 border border-slate-800 rounded-xl max-h-48 overflow-y-auto custom-scrollbar">

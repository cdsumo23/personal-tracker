...
export default function CategoriesPage() {
 ...
 />
  ...
  {isLoading ? (
    <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-4">
      {[1, 2, 3, 4, 5, 6].map((n) => (
        <Card key={n} className="h-24 animate-pulse bg-slate-100 dark:bg-slate-800/40 border border-slate-200 dark:border-slate-700" />
      ))}
    </div>
  ...
  <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-4">
    {filteredCategories.map((cat) => {
      ...
      return (
        <Card
          key={cat.id}
          className="flex items-center justify-between p-4 bg-white/60 dark:bg-slate-900/60 border-slate-200 dark:border-slate-800/80 hover:border-slate-300 dark:hover:border-slate-700/60 backdrop-blur-md transition-all group"
        >
          <div className="flex items-center gap-3">
            ...
            <div>
              <h4 className="font-semibold text-slate-900 dark:text-slate-200 text-sm">{cat.name}</h4>
              <div className="flex items-center gap-1.5 mt-0.5">
                {isSystem ? (
                  <Badge variant="default" className="text-[9px] px-1 py-0 bg-slate-100 dark:bg-slate-800 text-slate-500 dark:text-slate-400 border border-slate-200 dark:border-slate-700">
                    System
                  </Badge>
                ) : (
                  <Badge variant="success" className="text-[9px] px-1 py-0 bg-emerald-500/10 text-emerald-600 dark:text-emerald-400 border border-emerald-500/10">
                    Custom
                  </Badge>
                )}
              </div>
            ...
            {canModify ? (
              <>
                <button
                  ...
                  className="p-1.5 rounded-lg text-slate-500 dark:text-slate-400 hover:text-slate-800 dark:hover:text-slate-200 hover:bg-slate-100 dark:hover:bg-slate-800 transition-all"
                  title="Edit Category"
                >
              ...
              <span className="p-1.5 text-slate-400 cursor-not-allowed" title="System categories are read-only for standard users.">
 ...
 <form onSubmit={handleSubmit(onSubmit)} className="space-y-4 pt-2">
  {selectedCategory && selectedCategory.isSystem && isAdmin && (
    <div className="flex items-start gap-2.5 p-3 rounded-xl bg-amber-500/10 border border-amber-500/20 text-amber-600 dark:text-amber-400 text-xs mb-2 leading-relaxed">
 ...
 />
  ...
  <div className="space-y-2">
    ...
    <div className="flex flex-wrap gap-2 p-3 bg-slate-50 dark:bg-slate-950/40 border border-slate-200 dark:border-slate-800 rounded-xl">
  ...
  {/* ICON PICKER */}
  <div className="space-y-2">
    <label className="block text-sm font-medium text-slate-700 dark:text-slate-300">Category Icon</label>
    <div className="grid grid-cols-6 sm:grid-cols-8 gap-2 p-3 bg-slate-50 dark:bg-slate-950/40 border border-slate-200 dark:border-slate-800 rounded-xl max-h-48 overflow-y-auto custom-scrollbar">
      {Array.from(new Set(CATEGORY_ICONS.map((i) => i.icon))).map((iconName) => {
        ...
        return (
          <button
            ...
            className={`h-9 rounded-lg flex items-center justify-center hover:bg-slate-100 dark:hover:bg-slate-800 active:scale-95 transition-all ${
              isSelected

...
export default function NotificationsPage() {
  ...
  const getNotificationBg = (type: string, isRead: boolean) => {
    if (isRead) return 'bg-slate-900/10 border-slate-800/40 opacity-70';
    switch (type) {
      case 'SUCCESS':
  ...
  if (isLoading) {
    return (
      <div className="space-y-6 pb-20">
        ...
        <Card className="p-6 space-y-4 border-slate-800 bg-slate-900/60 backdrop-blur-md">
          {[...Array(4)].map((_, i) => (
            <div key={i} className="flex gap-4 p-4 border border-slate-800/80 rounded-xl animate-pulse">
  ...
  return (
    <div className="space-y-6 pb-20">
        ...
        action={
          hasUnread && (
            <Button
              ...
              className="border-slate-700 hover:bg-slate-800 text-slate-300 hover:text-slate-100 flex items-center gap-1.5 text-xs font-bold"
            >
      ...
      {/* Notifications List */}
      <Card className="border-slate-800 bg-slate-900/60 backdrop-blur-md overflow-hidden p-6 space-y-3">
        {filteredNotifications.length === 0 ? (
          ...
          <div className="divide-y divide-slate-800/50">
            {filteredNotifications.map((n) => (
              ...
              >
                {/* Icon Container */}
                <div className={`p-2 rounded-xl flex-shrink-0 bg-slate-950/40 border border-slate-800/80`}>
                ...
                <div className="flex-1 min-w-0">
                  <div className="flex items-center gap-2 flex-wrap">
                    <h4 className={`text-[13px] font-bold tracking-tight ${n.isRead ? 'text-slate-300' : 'text-slate-100'}`}>
                      {n.title}
                    </h4>
                    {!n.isRead && (
                ...
                <div className="flex items-center gap-1 opacity-0 group-hover:opacity-100 transition-opacity flex-shrink-0 self-center">
                  {!n.isRead && (
                    <button
                      ...
                      title="Mark as read"
                      className="p-1.5 rounded-lg text-slate-400 hover:text-emerald-400 hover:bg-emerald-500/10 transition-colors touch-target"
                    >
                  ...
                  <button
                    ...
                    className="p-1.5 rounded-lg text-slate-400 hover:text-rose-400 hover:bg-rose-500/10 transition-colors touch-target"
                  >

...
function TransactionRow({
  ...
  return (
    <div className="flex items-center gap-4 py-3.5 border-b border-slate-800/50 last:border-0 hover:bg-slate-800/20 px-2 rounded-xl transition-colors group">
      ...
      <div className="flex-1 min-w-0">
        ...
        <div className="flex items-center gap-2 flex-wrap mt-0.5">
          ...
          {tx.category && (
            <span className="text-[10px] text-slate-400 font-semibold">{tx.category.name}</span>
          )}
          {tx.account && (
          ...
          {tx.tags && tx.tags.length > 0 && tx.tags.map((tag) => (
            <span key={tag} className="text-[10px] bg-slate-800/80 text-slate-400 px-1.5 py-0.5 rounded-full">{tag}</span>
          ))}
        </div>
      </div>
    ...
    <div className="space-y-6">
      ...
      <Card className="flex flex-col sm:flex-row gap-4">
        <div className="relative flex-1">
          ...
          <input
            ...
            className="w-full pl-9 pr-3 py-2.5 bg-slate-800/60 border border-slate-700/50 rounded-xl text-sm text-slate-200 placeholder-slate-500 focus:outline-none focus:ring-1 focus:ring-primary-500 transition-all"
          />
          {search && (
        ...
        <Card className="grid grid-cols-2 md:grid-cols-4 gap-4">
          <div>
            ...
            <select
              ...
              className="w-full bg-slate-800/60 border border-slate-700/50 rounded-xl px-3 py-2 text-xs text-slate-200 focus:outline-none focus:ring-1 focus:ring-primary-500"
            >
          ...
          <div className="flex items-center justify-between mt-6 pt-4 border-t border-slate-800/50">
            ...
            <div className="flex items-center gap-2">
              <button
                ...
                className="p-2 rounded-xl text-slate-400 hover:text-slate-200 hover:bg-slate-800/60 disabled:opacity-30 disabled:cursor-not-allowed transition-all"
              >
      ...
      >
        <form onSubmit={handleSubmit(onSubmit)} className="space-y-4">
          ...
          <div className="grid grid-cols-2 gap-4">
            <div>
              <label className="block text-xs font-bold text-slate-400 uppercase tracking-wider mb-1.5">Account</label>
              <select
                {...register('accountId')}
                className="w-full bg-slate-800/60 border border-slate-700/50 rounded-xl px-3 py-2.5 text-sm text-slate-200 focus:outline-none focus:ring-1 focus:ring-primary-500"
              >
          ...
          <div className="flex items-center gap-2">
            ...
            <label htmlFor="isRecurring" className="text-xs font-semibold text-slate-300">Recurring transaction</label>
          </div>

          {watch('isRecurring') && (
            <div>
              <label className="block text-xs font-bold text-slate-400 uppercase tracking-wider mb-1.5">Interval</label>
              <select {...register('recurringInterval')} className="w-full bg-slate-800/60 border border-slate-700/50 rounded-xl px-3 py-2.5 text-sm text-slate-200 focus:outline-none focus:ring-1 focus:ring-primary-500">

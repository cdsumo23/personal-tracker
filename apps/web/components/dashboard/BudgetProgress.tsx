...
export function BudgetProgress() {
  ...
  if (!activeBudget || !usage) {
    return (
      <Card className="flex flex-col justify-between h-[380px] hover:border-slate-700/60 transition-all duration-300">
        <div>
        ...
        <div className="flex-1 flex flex-col items-center justify-center text-center p-6">
          <div className="w-14 h-14 rounded-full bg-slate-800/40 border border-slate-800 flex items-center justify-center mb-3 text-slate-500">
            <PiggyBank className="w-6 h-6 text-slate-400" />
          </div>
          <p className="text-xs font-semibold text-slate-400">No active budgets found</p>
          ...
          <Link
            href="/budgets"
            className="inline-flex items-center gap-1 text-xs font-bold text-primary-400 hover:text-primary-300 transition-colors"
          >
  ...
  return (
    <Card className="hover:border-slate-700/60 transition-all duration-300 flex flex-col justify-between h-[380px]">
      <div>
        <div className="flex justify-between items-start">
          <div>
            ...
            <p className="text-xs text-slate-500 mt-0.5">
              Active: <span className="text-slate-300 font-semibold">{activeBudget.name}</span>
            </p>
          </div>
          <Link
            href="/budgets"
        ...
        <div className="mt-4 p-3 rounded-2xl bg-slate-800/40 border border-slate-800/60 flex items-center justify-between">
          <div className="space-y-1">
            <p className="text-[10px] text-slate-500 uppercase font-bold tracking-wider">Overall Limit</p>
            <p className="text-xs text-slate-300 font-medium">
              <span className="text-sm font-extrabold text-slate-100">{format(spent)}</span> of {format(allocated)}
            </p>
          </div>
              ...
              <div key={cat.categoryId + idx} className="space-y-1.5">
                <div className="flex justify-between text-xs">
                  ...
                  <div className="text-slate-400 font-medium">
                    <span className="font-extrabold text-slate-200">{format(cat.spent)}</span>

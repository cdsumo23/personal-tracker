...
function TransactionRow({ tx, format }: { tx: Transaction; format: (n: number) => string }) {
  ...
  return (
    <div className="flex items-center gap-3 py-2.5 border-b border-slate-800/50 last:border-b-0 hover:bg-slate-800/20 px-1 rounded-xl transition-colors">
      ...
      <div className="flex-1 min-w-0">
        ...
        <p className="text-[10px] text-slate-500 mt-0.5">
          {tx.category?.name && <span className="text-slate-400 font-medium">{tx.category.name} · </span>}
    ...
    <Card className="hover:border-slate-700/60 transition-all duration-300">
      ...
      {transactions.length === 0 ? (
        <div className="flex flex-col items-center justify-center py-10 text-center">
          ...
          <Link href="/transactions" className="mt-3 text-xs font-bold text-primary-400 hover:text-primary-300 transition-colors">
            Add Transaction
          </Link>
        </div>
      ) : (

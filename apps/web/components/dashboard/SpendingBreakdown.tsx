...
export function SpendingBreakdown() {
 ...
 return (
 <div className="bg-white dark:bg-slate-900/90 border border-slate-200 dark:border-slate-800 p-2.5 rounded-xl shadow-xl backdrop-blur-md text-xs">
 ...
 <span className="font-extrabold text-slate-900 dark:text-slate-100">{format(amount)}</span>
 </p>
 <p className="flex justify-between items-center gap-4">
 <span className="text-slate-500 dark:text-slate-400 font-medium">Share:</span>
 <span className="font-extrabold text-slate-600 dark:text-slate-300">{percentage}%</span>
 </p>
 </div>
 );
 }
 ...
 <div className="w-16 h-16 rounded-full bg-slate-50 dark:bg-slate-800/40 border border-slate-200 dark:border-slate-800 flex items-center justify-center mb-3 text-slate-500">
 📊
 </div>
 ...
 <Cell key={`cell-${index}`} fill={entry.color} stroke="transparent" strokeWidth={1} />
 ))}
 </Pie>
 ...
 <span className="text-base font-black text-slate-900 dark:text-slate-100 tracking-tight mt-0.5">
 {format(total)}
 </span>
 </div>
 </div>
 ...
 />
 <span className="text-slate-600 dark:text-slate-300 font-semibold truncate group-hover:text-slate-900 dark:group-hover:text-slate-100 transition-colors">
 {item.name}
 </span>
 </div>
 <div className="text-right flex-shrink-0 ml-2">
 <span className="font-bold text-slate-800 dark:text-slate-200">{format(item.amount)}</span>

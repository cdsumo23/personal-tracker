...
function BillRow({ bill, format }: { bill: Bill; format: (n: number) => string }) {
  ...
  const statusConfig = bill.isPaid
    ...
    : { label: `${daysLeft}d`, color: 'text-slate-400', bg: 'bg-slate-800/60', icon: <CalendarClock className="w-3 h-3" /> };

  return (
    <div className="flex items-center justify-between py-2.5 border-b border-slate-800/50 last:border-0 hover:bg-slate-800/20 px-1 rounded-xl transition-colors">
      <div className="flex items-center gap-3 min-w-0">
        <div className="w-8 h-8 rounded-xl bg-slate-800/60 flex items-center justify-center flex-shrink-0 text-slate-400 text-sm">
          {bill.isPaid ? '✓' : '🧾'}
        </div>
      ...
      <div className="flex items-center gap-2 flex-shrink-0 ml-2">
        <span className="text-xs font-extrabold text-slate-200">{format(bill.amount)}</span>
...
export function UpcomingBills() {
  ...
  return (
    <Card className="hover:border-slate-700/60 transition-all duration-300">
      <div className="flex items-center justify-between mb-4">
        <div>
          ...
          <p className="text-xs text-slate-500 mt-0.5">
            ...
            <span className="text-slate-300 font-semibold">{format(totalDue)}</span>
            {overdueBills.length > 0 && (
              <span className="text-red-400 font-bold ml-2">· {overdueBills.length} overdue</span>
            )}
          </p>
        </div>
        <Link href="/bills" className="text-xs font-bold text-primary-400 hover:text-primary-300 transition-colors">
          Manage
        </Link>
      </div>
      ...
      {upcomingBills.length === 0 ? (
        <div className="flex flex-col items-center justify-center py-8 text-center">
          <div className="w-12 h-12 rounded-full bg-slate-800/40 border border-slate-800 flex items-center justify-center mb-3">
            <Bell className="w-5 h-5 text-slate-500" />
          </div>
          <p className="text-xs font-semibold text-slate-400">No upcoming bills</p>
          <p className="text-[10px] text-slate-500 mt-1">All bills are paid or no bills configured.</p>
          <Link href="/bills" className="mt-3 text-xs font-bold text-primary-400 hover:text-primary-300 transition-colors">
            Add Bills
          </Link>
        </div>
      ) : (
